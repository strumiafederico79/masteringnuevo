...
def run_reference_job(job_id: str, input_path: str, reference_path: str, params: dict):
    ...
    try:
        cleanup_old()
        result = process_audio_with_reference(input_path, reference_path, **params)
        jobs[job_id].update(status="done", result=result, finished_at=time.time())
...
async def ws_master_stream(websocket: WebSocket):
    ...
    try:
        ...
        chain_params = {k: v for k, v in config_msg.items() if k not in ("chunk_seconds", "preset", "platform_target", "type")}
...
async def preview(
    ...
    preview_seconds: float    = Query(15.0,  ge=5.0,   le=120.0),
...
async def master_async(
    ...
    if duration is not None:
        job_params["_input_duration_sec"] = duration
    jobs[job_id] = {"status": "queued", "filename": file.filename, "created_at": time.time(), "params": job_params}
    background_tasks.add_task(run_mastering_job, job_id, input_path, params)
...
def download(job_id: str):
    ...
    return FileResponse(output_path, media_type=mt, filename=f"mastered.{fmt}")

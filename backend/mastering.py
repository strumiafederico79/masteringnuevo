...
import uuid
import os
from scipy.signal import butter, sosfilt, sosfiltfilt, fftconvolve, resample_poly
...
def eq_high_shelf(audio: np.ndarray, sr: int,
    ...
    alpha = sin_w0 / 2.0 * np.sqrt((A + 1.0 / A) * (1.0 / 1.0 - 1.0) + 2.0)
    sqrtA = np.sqrt(A)
...
def multiband_stereo_width(audio: np.ndarray, sr: int,
    ...
    low_band  = sosfiltfilt(sos_lo_lp, audio)
    mh_band   = sosfiltfilt(sos_lo_hp, audio)
    mid_band  = sosfiltfilt(sos_hi_lp, mh_band)
    high_band = sosfiltfilt(sos_hi_hp, mh_band)
    ...
    def gain_curve(abs_signal):
        if ovs > 1:
            ...
            if la_up > 1:
                ...
                abs_up_la = np.roll(fwd_max, -(la_up // 2))
            else:
                abs_up_la = abs_up
            ...
            if lookahead_samples > 1:
                ...
                abs_ch_la = np.roll(fwd_max, -(lookahead_samples // 2))
            else:
                abs_ch_la = abs_ch
...
def _averaged_magnitude_spectrum(mono: np.ndarray, n_fft: int, max_frames: int = 64) -> np.ndarray:
    """Promedia la magnitud del espectro (FFT + ventana Hann) tomando frames
    DISTRIBUIDOS a lo largo de toda la señal, no solo los primeros segundos.

    BUGFIX importante: las funciones que usaban este patrón recorrían los
    frames secuencialmente desde el arranque del archivo y cortaban apenas
    juntaban `max_frames` (p.ej. 64 frames de n_fft=8192 a 44.1kHz son ~6
    segundos; 32 frames de 4096 son ~1.5s). Para un tema de 3-4 minutos eso
    significa que TODO el análisis tonal —incluida la curva de EQ de
    matching contra la referencia, que se arma a partir de este espectro—
    se calculaba casi exclusivamente con la intro. Ahora los frames se
    reparten uniformemente entre el segundo 0 y el final del archivo.
    """
    n = len(mono)
    ...
    max_start = n - n_fft
    # No más frames de los que entran sin solaparse más del 75% (n_fft/4 de paso mínimo)
    n_frames = int(np.clip(max_start // (n_fft // 4) + 1, 1, max_frames))
    starts = np.linspace(0, max_start, n_frames, dtype=int)
    mags = [np.abs(np.fft.rfft(mono[s:s + n_fft] * window)) for s in starts]
...
def spectrum_analysis_fft(audio: np.ndarray, sr: int,
    ...
    avg_mag = _averaged_magnitude_spectrum(mono, n_fft, max_frames=60)
    ...
    for i in range(n_bins):
        ...
        energy = float(np.mean(avg_mag[mask])) if mask.any() else 1e-9
        bin_db.append(round(20.0 * np.log10(energy + 1e-9), 2))
        bin_freq.append(round(float((lo + hi) * 0.5), 1))
...
def analyze_audio(audio: np.ndarray, sr: int) -> dict:
    ...
    avg_fft = _averaged_magnitude_spectrum(mono, n_fft, max_frames=32)
    freqs   = np.fft.rfftfreq(n_fft, d=1.0 / sr)
    ...
    for name, (lo, hi) in bands.items():
        mask   = (freqs >= lo) & (freqs < hi)
        energy = float(np.mean(avg_fft[mask]) + 1e-9) if mask.any() else 1e-9
        spectrum[name] = round(20.0 * np.log10(energy), 2)
...
MASTERING_PRESETS = {
    # ═══ Existentes, ahora con todos los parámetros de la cadena ═══
    "rock": {
        "label": "Rock",
        "input_gain_db": 0.0, "target_peak": 0.95, "use_lufs_normalize": False, "target_lufs": -9.0,
        "hp_cutoff": 40.0, "high_shelf_gain_db": 2.5,
        "eq1_freq": 100.0, "eq1_gain": 1.5, "eq1_q": 1.0,
        "eq2_freq": 400.0, "eq2_gain": -1.5, "eq2_q": 1.2,
        "eq3_freq": 3000.0, "eq3_gain": 2.0, "eq3_q": 1.0,
        "eq4_freq": 9000.0, "eq4_gain": 1.5, "eq4_q": 0.9,
        "comp_threshold": 0.5, "comp_ratio": 2.5, "comp_attack_ms": 10.0, "comp_release_ms": 120.0, "comp_makeup_db": 1.5,
        "transient_attack": 0.12, "transient_sustain": 0.05,
        "mb_bypass": False,
        "mb_low_crossover": 150.0, "mb_high_crossover": 4000.0,
        "mb_low_threshold": 0.55, "mb_low_ratio": 2.2, "mb_low_attack_ms": 25.0, "mb_low_release_ms": 150.0, "mb_low_makeup_db": 0.5,
        "mb_mid_threshold": 0.60, "mb_mid_ratio": 2.0, "mb_mid_attack_ms": 15.0, "mb_mid_release_ms": 120.0, "mb_mid_makeup_db": 0.5,
        "mb_high_threshold": 0.65, "mb_high_ratio": 1.8, "mb_high_attack_ms": 8.0,  "mb_high_release_ms": 90.0,  "mb_high_makeup_db": 0.3,
        "saturation_drive": 0.15, "saturation_mode": "tape", "saturation_mix": 0.30,
        ...
        "reverb_size": 0.25, "reverb_wet": 0.04,
        "limiter_ceiling": 0.96, "limiter_release_ms": 60.0,
    },
    "metal": {
        "label": "Metal",
        "input_gain_db": 0.0, "target_peak": 0.96, "use_lufs_normalize": False, "target_lufs": -8.0,
        "hp_cutoff": 50.0, "high_shelf_gain_db": 3.0,
        "eq1_freq": 90.0, "eq1_gain": 2.0, "eq1_q": 1.0,
        "eq2_freq": 300.0, "eq2_gain": -3.0, "eq2_q": 1.4,
        "eq3_freq": 2500.0, "eq3_gain": 3.0, "eq3_q": 1.1,
        "eq4_freq": 8000.0, "eq4_gain": 2.0, "eq4_q": 0.8,
        "comp_threshold": 0.45, "comp_ratio": 3.0, "comp_attack_ms": 8.0, "comp_release_ms": 100.0, "comp_makeup_db": 2.0,
        "transient_attack": 0.20, "transient_sustain": 0.0,
        "mb_bypass": False,
        "mb_low_crossover": 180.0, "mb_high_crossover": 3500.0,
        "mb_low_threshold": 0.50, "mb_low_ratio": 2.5, "mb_low_attack_ms": 20.0, "mb_low_release_ms": 140.0, "mb_low_makeup_db": 0.5,
        "mb_mid_threshold": 0.55, "mb_mid_ratio": 2.3, "mb_mid_attack_ms": 12.0, "mb_mid_release_ms": 100.0, "mb_mid_makeup_db": 0.8,
        "mb_high_threshold": 0.60, "mb_high_ratio": 2.0, "mb_high_attack_ms": 6.0,  "mb_high_release_ms": 70.0,  "mb_high_makeup_db": 0.5,
        "saturation_drive": 0.25, "saturation_mode": "tube", "saturation_mix": 0.35,
        "mid_gain_db": 0.0, "side_gain_db": 0.0, "stereo_width_amount": 1.0,
        "use_stereo_enhancer": False, "enhancer_bass_mono_freq": 120.0, "haas_delay_ms": 0.0,
        "reverb_size": 0.20, "reverb_wet": 0.02,
        "limiter_ceiling": 0.97, "limiter_release_ms": 45.0,
    },
    "trap": {
        "label": "Trap",
        "input_gain_db": 0.0, "target_peak": 0.97, "use_lufs_normalize": False, "target_lufs": -6.5,
        "hp_cutoff": 25.0, "high_shelf_gain_db": 2.5,
        "eq1_freq": 60.0, "eq1_gain": 3.0, "eq1_q": 0.9,
        "eq2_freq": 250.0, "eq2_gain": -2.0, "eq2_q": 1.2,
        "eq3_freq": 3500.0, "eq3_gain": 2.5, "eq3_q": 1.0,
        "eq4_freq": 10000.0, "eq4_gain": 2.5, "eq4_q": 0.8,
        "comp_threshold": 0.40, "comp_ratio": 3.5, "comp_attack_ms": 5.0, "comp_release_ms": 90.0, "comp_makeup_db": 2.0,
        "transient_attack": 0.25, "transient_sustain": -0.10,
        "mb_bypass": False,
        "mb_low_crossover": 100.0, "mb_high_crossover": 4500.0,
        "mb_low_threshold": 0.45, "mb_low_ratio": 3.0, "mb_low_attack_ms": 15.0, "mb_low_release_ms": 180.0, "mb_low_makeup_db": 1.0,
        "mb_mid_threshold": 0.60, "mb_mid_ratio": 2.0, "mb_mid_attack_ms": 10.0, "mb_mid_release_ms": 100.0, "mb_mid_makeup_db": 0.5,
        "mb_high_threshold": 0.65, "mb_high_ratio": 1.8, "mb_high_attack_ms": 5.0,  "mb_high_release_ms": 60.0,  "mb_high_makeup_db": 0.3,
        "saturation_drive": 0.10, "saturation_mode": "tape", "saturation_mix": 0.20,
        "mid_gain_db": 0.0, "side_gain_db": 0.0, "stereo_width_amount": 1.10,
        "use_stereo_enhancer": True, "enhancer_bass_mono_freq": 100.0, "haas_delay_ms": 4.0,
        "reverb_size": 0.20, "reverb_wet": 0.0,
        "limiter_ceiling": 0.98, "limiter_release_ms": 35.0,
    },
    "rap": {
        "label": "Rap / Hip-Hop",
        "input_gain_db": 0.0, "target_peak": 0.96, "use_lufs_normalize": False, "target_lufs": -8.0,
        "hp_cutoff": 35.0, "high_shelf_gain_db": 1.5,
        "eq1_freq": 80.0, "eq1_gain": 2.5, "eq1_q": 0.9,
        "eq2_freq": 350.0, "eq2_gain": -1.5, "eq2_q": 1.1,
        "eq3_freq": 2000.0, "eq3_gain": 2.0, "eq3_q": 1.2,
        "eq4_freq": 7000.0, "eq4_gain": 1.5, "eq4_q": 0.9,
        "comp_threshold": 0.50, "comp_ratio": 2.5, "comp_attack_ms": 10.0, "comp_release_ms": 110.0, "comp_makeup_db": 1.5,
        "transient_attack": 0.15, "transient_sustain": 0.0,
        "mb_bypass": False,
        "mb_low_crossover": 120.0, "mb_high_crossover": 4000.0,
        "mb_low_threshold": 0.50, "mb_low_ratio": 2.5, "mb_low_attack_ms": 18.0, "mb_low_release_ms": 160.0, "mb_low_makeup_db": 0.6,
        "mb_mid_threshold": 0.60, "mb_mid_ratio": 2.0, "mb_mid_attack_ms": 12.0, "mb_mid_release_ms": 110.0, "mb_mid_makeup_db": 0.5,
        "mb_high_threshold": 0.65, "mb_high_ratio": 1.8, "mb_high_attack_ms": 6.0,  "mb_high_release_ms": 80.0,  "mb_high_makeup_db": 0.3,
        ...
        "reverb_size": 0.20, "reverb_wet": 0.03,
        "limiter_ceiling": 0.96, "limiter_release_ms": 55.0,
    },
    "reggaeton": {
        "label": "Reggaeton",
        "input_gain_db": 0.0, "target_peak": 0.97, "use_lufs_normalize": False, "target_lufs": -7.0,
        "hp_cutoff": 30.0, "high_shelf_gain_db": 2.5,
        "eq1_freq": 70.0, "eq1_gain": 3.0, "eq1_q": 0.9,
        "eq2_freq": 300.0, "eq2_gain": -2.0, "eq2_q": 1.2,
        "eq3_freq": 3000.0, "eq3_gain": 2.5, "eq3_q": 1.0,
        "eq4_freq": 9000.0, "eq4_gain": 2.0, "eq4_q": 0.8,
        "comp_threshold": 0.45, "comp_ratio": 3.0, "comp_attack_ms": 8.0, "comp_release_ms": 100.0, "comp_makeup_db": 1.8,
        "transient_attack": 0.20, "transient_sustain": -0.05,
        "mb_bypass": False,
        "mb_low_crossover": 110.0, "mb_high_crossover": 4200.0,
        "mb_low_threshold": 0.48, "mb_low_ratio": 2.8, "mb_low_attack_ms": 16.0, "mb_low_release_ms": 170.0, "mb_low_makeup_db": 0.8,
        "mb_mid_threshold": 0.58, "mb_mid_ratio": 2.2, "mb_mid_attack_ms": 10.0, "mb_mid_release_ms": 110.0, "mb_mid_makeup_db": 0.5,
        "mb_high_threshold": 0.63, "mb_high_ratio": 2.0, "mb_high_attack_ms": 6.0,  "mb_high_release_ms": 75.0,  "mb_high_makeup_db": 0.4,
        "saturation_drive": 0.12, "saturation_mode": "tape", "saturation_mix": 0.25,
        "mid_gain_db": 0.0, "side_gain_db": 0.0, "stereo_width_amount": 1.15,
        "use_stereo_enhancer": True, "enhancer_bass_mono_freq": 110.0, "haas_delay_ms": 3.0,
        "reverb_size": 0.15, "reverb_wet": 0.02,
        "limiter_ceiling": 0.97, "limiter_release_ms": 40.0,
    },
    "pop": {
        "label": "Pop",
        "input_gain_db": 0.0, "target_peak": 0.96, "use_lufs_normalize": False, "target_lufs": -10.0,
        "hp_cutoff": 40.0, "high_shelf_gain_db": 2.5,
        "eq1_freq": 100.0, "eq1_gain": 1.0, "eq1_q": 1.0,
        "eq2_freq": 400.0, "eq2_gain": -1.0, "eq2_q": 1.1,
        "eq3_freq": 3000.0, "eq3_gain": 2.0, "eq3_q": 1.0,
        "eq4_freq": 10000.0, "eq4_gain": 2.5, "eq4_q": 0.8,
        "comp_threshold": 0.55, "comp_ratio": 2.2, "comp_attack_ms": 12.0, "comp_release_ms": 130.0, "comp_makeup_db": 1.2,
        "transient_attack": 0.10, "transient_sustain": 0.0,
        "mb_bypass": False,
        "mb_low_crossover": 200.0, "mb_high_crossover": 4000.0,
        "mb_low_threshold": 0.60, "mb_low_ratio": 2.0, "mb_low_attack_ms": 22.0, "mb_low_release_ms": 150.0, "mb_low_makeup_db": 0.4,
        "mb_mid_threshold": 0.65, "mb_mid_ratio": 1.8, "mb_mid_attack_ms": 14.0, "mb_mid_release_ms": 120.0, "mb_mid_makeup_db": 0.4,
        "mb_high_threshold": 0.68, "mb_high_ratio": 1.6, "mb_high_attack_ms": 8.0,  "mb_high_release_ms": 90.0,  "mb_high_makeup_db": 0.3,
        "saturation_drive": 0.08, "saturation_mode": "tape", "saturation_mix": 0.20,
        "mid_gain_db": 0.0, "side_gain_db": 0.0, "stereo_width_amount": 1.10,
        "use_stereo_enhancer": True, "enhancer_bass_mono_freq": 120.0, "haas_delay_ms": 3.0,
        "reverb_size": 0.30, "reverb_wet": 0.06,
        "limiter_ceiling": 0.96, "limiter_release_ms": 65.0,
    },
    "cd": {
        "label": "CD / Audiófilo",
        "input_gain_db": 0.0, "target_peak": 0.90, "use_lufs_normalize": False, "target_lufs": -16.0,
        "hp_cutoff": 25.0, "high_shelf_gain_db": 1.0,
        "eq1_freq": 100.0, "eq1_gain": 0.0, "eq1_q": 1.0,
        "eq2_freq": 500.0, "eq2_gain": 0.0, "eq2_q": 1.0,
        "eq3_freq": 2000.0, "eq3_gain": 0.5, "eq3_q": 1.0,
        "eq4_freq": 9000.0, "eq4_gain": 0.5, "eq4_q": 0.9,
        "comp_threshold": 0.80, "comp_ratio": 1.6, "comp_attack_ms": 20.0, "comp_release_ms": 180.0, "comp_makeup_db": 0.0,
        ...
        "mb_low_threshold": 0.85, "mb_low_ratio": 1.5, "mb_low_attack_ms": 30.0, "mb_low_release_ms": 200.0, "mb_low_makeup_db": 0.0,
        "mb_mid_threshold": 0.85, "mb_mid_ratio": 1.5, "mb_mid_attack_ms": 20.0, "mb_mid_release_ms": 180.0, "mb_mid_makeup_db": 0.0,
        "mb_high_threshold": 0.85, "mb_high_ratio": 1.4, "mb_high_attack_ms": 10.0, "mb_high_release_ms": 120.0, "mb_high_makeup_db": 0.0,
        "saturation_drive": 0.0, "saturation_mode": "tape", "saturation_mix": 0.0,
        "mid_gain_db": 0.0, "side_gain_db": 0.0, "stereo_width_amount": 1.0,
        "use_stereo_enhancer": False, "enhancer_bass_mono_freq": 120.0, "haas_delay_ms": 0.0,
        "reverb_size": 0.30, "reverb_wet": 0.02,
        "limiter_ceiling": 0.92, "limiter_release_ms": 100.0,
    },

    # ═══ Nuevos géneros ═══
    "edm": {
        "label": "EDM / House",
        "input_gain_db": 0.0, "target_peak": 0.98, "use_lufs_normalize": False, "target_lufs": -6.0,
        "hp_cutoff": 30.0, "high_shelf_gain_db": 3.0,
        "eq1_freq": 60.0, "eq1_gain": 2.5, "eq1_q": 0.9,
        "eq2_freq": 250.0, "eq2_gain": -1.5, "eq2_q": 1.2,
        "eq3_freq": 3500.0, "eq3_gain": 2.0, "eq3_q": 1.0,
        "eq4_freq": 11000.0, "eq4_gain": 3.0, "eq4_q": 0.8,
        "comp_threshold": 0.40, "comp_ratio": 3.5, "comp_attack_ms": 6.0, "comp_release_ms": 90.0, "comp_makeup_db": 2.0,
        "transient_attack": 0.20, "transient_sustain": -0.10,
        "mb_bypass": False,
        "mb_low_crossover": 100.0, "mb_high_crossover": 5000.0,
        "mb_low_threshold": 0.42, "mb_low_ratio": 3.2, "mb_low_attack_ms": 12.0, "mb_low_release_ms": 180.0, "mb_low_makeup_db": 1.0,
        "mb_mid_threshold": 0.60, "mb_mid_ratio": 2.2, "mb_mid_attack_ms": 8.0,  "mb_mid_release_ms": 90.0,  "mb_mid_makeup_db": 0.5,
        "mb_high_threshold": 0.65, "mb_high_ratio": 2.0, "mb_high_attack_ms": 5.0,  "mb_high_release_ms": 60.0,  "mb_high_makeup_db": 0.4,
        "saturation_drive": 0.10, "saturation_mode": "tube", "saturation_mix": 0.20,
        "mid_gain_db": 0.0, "side_gain_db": 1.0, "stereo_width_amount": 1.20,
        "use_stereo_enhancer": True, "enhancer_bass_mono_freq": 100.0, "haas_delay_ms": 5.0,
        "reverb_size": 0.15, "reverb_wet": 0.02,
        "limiter_ceiling": 0.98, "limiter_release_ms": 30.0,
    },
    "techno": {
        "label": "Techno",
        "input_gain_db": 0.0, "target_peak": 0.98, "use_lufs_normalize": False, "target_lufs": -6.0,
        "hp_cutoff": 35.0, "high_shelf_gain_db": 2.0,
        "eq1_freq": 55.0, "eq1_gain": 2.0, "eq1_q": 0.9,
        "eq2_freq": 220.0, "eq2_gain": -2.0, "eq2_q": 1.3,
        "eq3_freq": 3000.0, "eq3_gain": 1.5, "eq3_q": 1.0,
        "eq4_freq": 9000.0, "eq4_gain": 1.0, "eq4_q": 0.8,
        "comp_threshold": 0.40, "comp_ratio": 3.5, "comp_attack_ms": 6.0, "comp_release_ms": 90.0, "comp_makeup_db": 2.0,
        "transient_attack": 0.15, "transient_sustain": -0.15,
        "mb_bypass": False,
        "mb_low_crossover": 90.0, "mb_high_crossover": 4500.0,
        "mb_low_threshold": 0.40, "mb_low_ratio": 3.5, "mb_low_attack_ms": 10.0, "mb_low_release_ms": 170.0, "mb_low_makeup_db": 1.2,
        "mb_mid_threshold": 0.58, "mb_mid_ratio": 2.3, "mb_mid_attack_ms": 8.0,  "mb_mid_release_ms": 90.0,  "mb_mid_makeup_db": 0.5,
        "mb_high_threshold": 0.62, "mb_high_ratio": 2.0, "mb_high_attack_ms": 5.0,  "mb_high_release_ms": 60.0,  "mb_high_makeup_db": 0.3,
        "saturation_drive": 0.15, "saturation_mode": "tube", "saturation_mix": 0.25,
        "mid_gain_db": 0.0, "side_gain_db": 0.0, "stereo_width_amount": 1.05,
    ...
    "rnb": {
        "label": "R&B / Soul",
        "input_gain_db": 0.0, "target_peak": 0.95, "use_lufs_normalize": False, "target_lufs": -10.0,
        "hp_cutoff": 35.0, "high_shelf_gain_db": 1.8,
        "eq1_freq": 90.0, "eq1_gain": 1.5, "eq1_q": 0.9,
        "eq2_freq": 350.0, "eq2_gain": -1.2, "eq2_q": 1.1,
        "eq3_freq": 2500.0, "eq3_gain": 1.5, "eq3_q": 1.0,
        "eq4_freq": 8000.0, "eq4_gain": 1.8, "eq4_q": 0.9,
        "comp_threshold": 0.55, "comp_ratio": 2.0, "comp_attack_ms": 14.0, "comp_release_ms": 140.0, "comp_makeup_db": 1.2,
        "transient_attack": 0.08, "transient_sustain": 0.05,
        "mb_bypass": False,
        "mb_low_crossover": 150.0, "mb_high_crossover": 3800.0,
        "mb_low_threshold": 0.55, "mb_low_ratio": 2.2, "mb_low_attack_ms": 22.0, "mb_low_release_ms": 160.0, "mb_low_makeup_db": 0.5,
        "mb_mid_threshold": 0.60, "mb_mid_ratio": 1.9, "mb_mid_attack_ms": 16.0, "mb_mid_release_ms": 130.0, "mb_mid_makeup_db": 0.6,
        "mb_high_threshold": 0.68, "mb_high_ratio": 1.6, "mb_high_attack_ms": 9.0,  "mb_high_release_ms": 90.0,  "mb_high_makeup_db": 0.3,
        "saturation_drive": 0.18, "saturation_mode": "tape", "saturation_mix": 0.35,
        "mid_gain_db": 0.0, "side_gain_db": 0.5, "stereo_width_amount": 1.05,
        "use_stereo_enhancer": True, "enhancer_bass_mono_freq": 110.0, "haas_delay_ms": 3.0,
        "reverb_size": 0.30, "reverb_wet": 0.06,
        "limiter_ceiling": 0.95, "limiter_release_ms": 90.0,
    },
    "jazz": {
        "label": "Jazz",
        "input_gain_db": 0.0, "target_peak": 0.93, "use_lufs_normalize": False, "target_lufs": -16.0,
        "hp_cutoff": 25.0, "high_shelf_gain_db": 1.2,
        "eq1_freq": 100.0, "eq1_gain": 0.5, "eq1_q": 1.0,
        "eq2_freq": 450.0, "eq2_gain": -0.5, "eq2_q": 1.0,
        "eq3_freq": 2500.0, "eq3_gain": 0.8, "eq3_q": 1.0,
        "eq4_freq": 8000.0, "eq4_gain": 1.0, "eq4_q": 0.9,
        "comp_threshold": 0.75, "comp_ratio": 1.6, "comp_attack_ms": 20.0, "comp_release_ms": 180.0, "comp_makeup_db": 0.5,
        "transient_attack": 0.0, "transient_sustain": 0.05,
        "mb_bypass": False,
        "mb_low_crossover": 180.0, "mb_high_crossover": 4000.0,
        "mb_low_threshold": 0.80, "mb_low_ratio": 1.5, "mb_low_attack_ms": 30.0, "mb_low_release_ms": 200.0, "mb_low_makeup_db": 0.0,
        "mb_mid_threshold": 0.82, "mb_mid_ratio": 1.5, "mb_mid_attack_ms": 22.0, "mb_mid_release_ms": 180.0, "mb_mid_makeup_db": 0.0,
        "mb_high_threshold": 0.85, "mb_high_ratio": 1.4, "mb_high_attack_ms": 12.0, "mb_high_release_ms": 130.0, "mb_high_makeup_db": 0.0,
        "saturation_drive": 0.08, "saturation_mode": "tape", "saturation_mix": 0.20,
        "mid_gain_db": 0.0, "side_gain_db": 0.0, "stereo_width_amount": 1.0,
        "use_stereo_enhancer": False, "enhancer_bass_mono_freq": 120.0, "haas_delay_ms": 0.0,
        "reverb_size": 0.40, "reverb_wet": 0.08,
        "limiter_ceiling": 0.93, "limiter_release_ms": 150.0,
    },
    "classical": {
        "label": "Clásica / Orquesta",
        "input_gain_db": 0.0, "target_peak": 0.89, "use_lufs_normalize": False, "target_lufs": -18.0,
        "hp_cutoff": 20.0, "high_shelf_gain_db": 0.5,
        "eq1_freq": 100.0, "eq1_gain": 0.0, "eq1_q": 1.0,
        "eq2_freq": 500.0, "eq2_gain": 0.0, "eq2_q": 1.0,
        "eq3_freq": 2500.0, "eq3_gain": 0.3, "eq3_q": 1.0,
        "eq4_freq": 9000.0, "eq4_gain": 0.3, "eq4_q": 0.9,
        "comp_threshold": 0.85, "comp_ratio": 1.4, "comp_attack_ms": 25.0, "comp_release_ms": 200.0, "comp_makeup_db": 0.0,
        "transient_attack": 0.0, "transient_sustain": 0.0,
        ...
        "mb_low_threshold": 0.88, "mb_low_ratio": 1.3, "mb_low_attack_ms": 40.0, "mb_low_release_ms": 250.0, "mb_low_makeup_db": 0.0,
        "mb_mid_threshold": 0.88, "mb_mid_ratio": 1.3, "mb_mid_attack_ms": 30.0, "mb_mid_release_ms": 220.0, "mb_mid_makeup_db": 0.0,
        "mb_high_threshold": 0.90, "mb_high_ratio": 1.2, "mb_high_attack_ms": 15.0, "mb_high_release_ms": 150.0, "mb_high_makeup_db": 0.0,
        "saturation_drive": 0.0, "saturation_mode": "tape", "saturation_mix": 0.0,
        "mid_gain_db": 0.0, "side_gain_db": 0.0, "stereo_width_amount": 1.0,
        "use_stereo_enhancer": False, "enhancer_bass_mono_freq": 120.0, "haas_delay_ms": 0.0,
        "reverb_size": 0.50, "reverb_wet": 0.10,
        "limiter_ceiling": 0.89, "limiter_release_ms": 250.0,
    },
    "lofi": {
        "label": "Lo-Fi",
        "input_gain_db": 0.0, "target_peak": 0.94, "use_lufs_normalize": False, "target_lufs": -11.0,
        "hp_cutoff": 45.0, "high_shelf_gain_db": -3.0,
        "eq1_freq": 80.0, "eq1_gain": -1.0, "eq1_q": 0.9,
        "eq2_freq": 300.0, "eq2_gain": 1.0, "eq2_q": 1.1,
        "eq3_freq": 2000.0, "eq3_gain": -1.5, "eq3_q": 1.0,
        "eq4_freq": 6000.0, "eq4_gain": -3.0, "eq4_q": 0.8,
        "comp_threshold": 0.55, "comp_ratio": 2.0, "comp_attack_ms": 16.0, "comp_release_ms": 140.0, "comp_makeup_db": 1.0,
        "transient_attack": 0.05, "transient_sustain": 0.20,
        "mb_bypass": False,
        "mb_low_crossover": 140.0, "mb_high_crossover": 3500.0,
        "mb_low_threshold": 0.60, "mb_low_ratio": 2.0, "mb_low_attack_ms": 25.0, "mb_low_release_ms": 170.0, "mb_low_makeup_db": 0.3,
        "mb_mid_threshold": 0.55, "mb_mid_ratio": 2.2, "mb_mid_attack_ms": 18.0, "mb_mid_release_ms": 140.0, "mb_mid_makeup_db": 0.5,
        "mb_high_threshold": 0.70, "mb_high_ratio": 1.5, "mb_high_attack_ms": 10.0, "mb_high_release_ms": 100.0, "mb_high_makeup_db": 0.0,
        "saturation_drive": 0.35, "saturation_mode": "tape", "saturation_mix": 0.50,
        "mid_gain_db": 0.0, "side_gain_db": -1.0, "stereo_width_amount": 0.90,
        "use_stereo_enhancer": False, "enhancer_bass_mono_freq": 130.0, "haas_delay_ms": 0.0,
        "reverb_size": 0.35, "reverb_wet": 0.10,
        "limiter_ceiling": 0.94, "limiter_release_ms": 120.0,
    },
}
...
# Orden de la cadena (todas las etapas excepto HPF, compresor de banda ancha
# y limiter son opcionales):
#   1. High-pass (limpieza de sub-graves, fase cero)
#   2. EQ paramétrico (4 bandas RBJ) + high shelf
#   3. Transient shaper
#   4. Compresor de banda ancha / "un solo cuerpo" (Threshold/Ratio/Attack/
#      Release/Makeup, siempre activo — antes era un control fantasma sin
#      efecto real; oversampling x4)
#   5. Compresor multibanda (bypass por defecto)
#   6. Saturación armónica (tape/tube, oversampling x4)
#   7. Mid/Side gain
#   8. Estéreo (enhancer o width simple)
#   9. Reverb (glue, sutil)
#   10. Limitador brick-wall con lookahead (siempre activo, último en la cadena)
...
def apply_mastering_chain(
    ...
    **_ignored,
    ...
    """
    Cadena de mastering: input gain → HPF → EQ → transient → multibanda →
    saturación → mid/side → estéreo → reverb → limitador. No se aplica
    ...
    # ── 4. Compresor de banda ancha ("un solo cuerpo": Threshold/Ratio/    ──
    ...
    # ── 8. Estéreo: enhancer o width simple (opcional) ─────────────────────
    if audio.shape[0] == 2:
    ...
    # ── 8b. Multiband Stereo Width (opcional, bypass por defecto) ──────────
...
def spectral_energy_at_bands(audio: np.ndarray, sr: int, band_edges: list) -> list:
    ...
    avg_mag = _averaged_magnitude_spectrum(mono, n_fft, max_frames=96)
    freqs = np.fft.rfftfreq(n_fft, d=1.0 / sr)
    out = []
    for lo, hi in band_edges:
        mask = (freqs >= lo) & (freqs < hi)
        energy = float(np.mean(avg_mag[mask]) + 1e-9) if mask.any() else 1e-9
        out.append(20.0 * np.log10(energy))
    return out
